  ...
  <v-dialog v-model="dialog" persistent max-width="800px">
    <template v-slot:activator="{ on }">
      <v-btn v-on="on" color="blue-grey" class="white--text">
        Edit Columns
        <v-icon right dark>mdi-table-edit</v-icon>
      </v-btn>
    </template>
    <v-card>
      <v-card-title>
      ...
      <v-card-actions>
        ...
        <v-btn color="blue darken-1" text @click="updateDefintion()">Save</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods: {
    async updateDefinition(){
        this.$store.dispatch("updateCurrentListDefinition", this.defintion);
        dialog = false;
    }
  }
};
